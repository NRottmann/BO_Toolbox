...
for i=1:params.maxIter
    ...
    f_min = min(f, [], 2);
    f_max = max(f, [], 2);
    % avoid length of 0 and division by 0 + no cut directly at the kown
    % features
    f_length = f_max-f_min;
    f_length(f_length==0) = min(f_length(f_length>0)) * ones(1, sum(f_length==0));
    f_min = f_min - 0.05 * f_length;
    f_max = f_max + 0.05 * f_length;
    f = (f-f_min)./repmat(f_max-f_min, 1, size(f,2));
...
% Performs the optimization of the hyperparmeters, including the parameters
% of the feature generator the the GP model by minimizing the 
%
