function [K,varargout] = se_kernel_var_HIBO(X1,X2,varargin)
...
DELTA = diag((params.CovParam(2:D+1)));
for i=1:1:(D-1)
    DELTA(end,i) = params.CovParam(D+1+i);
    DELTA(i,end) = params.CovParam(2*D+i);
end
DELTA = inv(DELTA);
